...
def get_api_key(api_key_header: str = Depends(dependency.api_key_header_auth)):
    ...
    if not api_key_data or not api_key_data.enabled or api_key_data.type != dependency.ExternalServices.train_microservice.name:
...
def register_dataset(dataset: MicroserviceConnection):
    ...
    try:
        r = requests.get(dataset.socket + '/status')
        r.raise_for_status()
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout, requests.exceptions.HTTPError):
        return {
            "status": "failure",
    ...
    settings.available_datasets[dataset.name] = dataset.socket
